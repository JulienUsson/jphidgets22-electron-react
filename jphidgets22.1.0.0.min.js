...
; (function () {
  ...
  */
    ...
    function debug(msg) {
        ...
        if (loglevel === 0) console.log('debug(' + tm() + '): ' + msg);
    }
    ...
    function getPhidgetConstructor(ch) {
        ...
        if (root.jPhidgets[ch.cname]) return root.jPhidgets[ch.cname];
        if (ch.cname.startsWith('Phidget')) {
            var cname = ch.cname.substring(7);
            if (root.jPhidgets[cname]) return root.jPhidgets[cname];
        }
        return null;
    }

    var jPhidgets = function jPhidgets() {

        var self = {};

        var NET_NAME = "jPhidget";
        if (isNode) var NET_TYPE = "www,nodejs"; else var NET_TYPE = "www";
        var NET_MAJOR = 2;
        var NET_MINOR = 0;
   ...
   */
        ...
        var ControlPanel = function ControlPanel(div) {
            ...
            this.mkDiv = function (obj, arg2) {
                ...
                if (obj instanceof Connection) return 'conn_' + this.basediv + '_' + obj.id; else if (obj instanceof Channel) return 'ch_' + this.basediv + '_' + obj.conn.id + '_' + obj.chid; else if (obj instanceof Device) return 'dev_' + this.basediv + '_' + obj.conn.id + '_' + obj.phid; else if (obj === 'port') return 'vint_port_' + arg2.parent.phid + '_' + arg2.conn.id + '_' + arg2.hubPort;
            };
            ...
            if (!$.jstree) throw "missing jquery jstree";

            $(this.userdiv).html('<div class="PhidgetControlPanel" id="' + this.basediv + '">' + '<div class="PhidgetControlPanelHeader" id="' + this.basediv + '_header"></div>' + '<div class="PhidgetControlPanelBody" id="' + this.basediv + '_body"></div>' + '<div class="PhidgetControlPanelFooter" id="' + this.basediv + '_footer"></div>' + '<div id="' + this.basediv + '_dialogs"></div>' + '</div>');
                ...
                contextmenu: {
                    items: function items(node) {
                        if (node.id.startsWith('ch_')) {
                            if (!node.data || !node.data.obj) return {};
                            return {
                                open: {
                                    label: 'Open',
    ...
    */
      ...
      */
            for (var c in Connections) {
                var mgr = managers[Connections[c].id] = Connections[c].createManager({
                    onDeviceAttach: function onDeviceAttach(dev) {
                        self.add(dev);
                    },
                    onDeviceDetach: function onDeviceDetach(dev) {
                        self.remove(dev);
                    },
                    onAttach: function onAttach(ch) {
                        self.add(ch);
                    },
                    onDetach: function onDetach(ch) {
                        self.remove(ch);
                    }
                });
                mgr.open();
        ...
        ControlPanel.prototype.openChannel = function (node) {
            ...
            if (cons) {
                ...
                phid.onError = function (err) {
                    ...
                    if (err.phidgetReturnCode == 7) console.log("Unable to open Phidget: Is it already open elsewhere?"); else console.log(err);
                };
                phid.onAttach = function (phid) {
                    this.render(phid);
                }.bind(this);
                phid.onDetach = function (phid) {
                    if (!phid.isopen) return;
     ...
     */
            ...
            if (ppdiv.length) {
                ...
                if (!example) throw phid.channel.cname + 'Example not found';
                example.setup(phid);
   ...
   */
    ...
    */
            ...
            if (obj.channelname && obj.channelname == "PhidgetHub") return;

            if (obj instanceof Channel) {
                var ch = obj;

                if (ch.parent.isHub()) return;
                ...
                if (node = jstree.create_node(parent, node, 'last')) jstree.get_node(node).data = { obj: ch }; else throw 'failed to add new node for channel:' + ch;
     ...
     */
                ...
                if (dev.label) var txt = dev.name + '(' + dev.label + ')'; else var txt = dev.name;
                ...
                if (dev.type === 'VINT') {
                    ...
                    if (!port) {
                        ...
                        if (!portid) throw 'failed to add new hub port node';
                    }
                    if (node = jstree.create_node(portid, node, 'last')) jstree.get_node(node).data = { obj: dev }; else throw 'failed to add new node for vint device:' + dev;
                } else {
                    if (dev.parent) var parent = '#' + this.mkDiv(dev.parent); else var parent = '#' + this.mkDiv(dev.conn);
                    if (node = jstree.create_node(parent, node, 'last')) jstree.get_node(node).data = { obj: dev }; else throw 'failed to add new node for device:' + dev;
                }
        ...
        ControlPanel.prototype.remove = function (obj) {
            ...
            if (obj instanceof Channel || obj instanceof Device) {
                ...
                if (obj instanceof Channel && obj.parent.isHub()) return;
     ...
     */
                ...
                if (node.data.phidget) delete node.data.phidget;
     ...
     */
                if (obj instanceof Device && obj.type === 'VINT') {
                    ...
                    if (node === false) return;
                    if (jstree.is_leaf(node)) jstree.delete_node(node);
                }
            }
        };
   ...
   */
        var Request = function Request(dataOrLength, flags, reqseq, repseq, type, stype) {

            if (typeof stype !== 'undefined') this.buffer = this.render(dataOrLength, flags, reqseq, repseq, type, stype); else if (typeof dataOrLength !== 'undefined') this.hdrlen = this.parse(dataOrLength);
        };

        Request.prototype.toString = function () {

            return '{flags:0x' + this.flags.toString(16) + ' len:' + this.len + ' reqseq:' + this.reqseq + ' repseq:' + this.repseq + ' type:' + this.type + ' subtype:' + this.stype + '}';
        };
        ...
        Request.prototype.parse = function (buf) {
            ...
            if (this.magic != NR.Magic) throw "Bad Request Magic";
   ...
   */
        ...
        var BridgePacket = function BridgePacket(conn, req, data) {
        ...
        BridgePacket.prototype.isEvent = function () {
            ...
            if (this.flags & this.F_EVENT) return true;
            return false;
        };
        ...
        BridgePacket.prototype.convertType = function (e) {
            ...
            switch (this.entryType(e)) {
                ...
                case this.STRING:
                    ...
                    return e.value;
                case this.JSON:
                    if (_typeof(e.value) === 'object') return e.value;
                    break;
                case this.ARRAY:
        ...
        BridgePacket.prototype.set = function (val) {
            ...
            if (typeof val === 'undefined') throw 'invalid argument';
            if (typeof val.name === 'undefined') throw 'missing name';
            if (typeof val.type === 'undefined') throw 'missing type';
            if (typeof val.value === 'undefined') throw 'missing value';

            if (val.name in this.entries) throw 'value [' + val.name + '] already set';

            if (!this.validType(val.type)) throw 'invalid type [' + val.type + ']';
        ...
        BridgePacket.prototype.send = function (ch, vpkt) {
            ...
            if (typeof vpkt === 'string') vpkt = this[vpkt];

            if (!ch.isopen) throw 'channel is not open';
        ...
        BridgePacket.prototype.deliver = function () {
            ...
            if (!this.channel) throw 'Bridge packet missing channel';
            ...
            try {
                ...
                if (!this.isEvent() && this.vpkt !== 0) this.conn.sendReply(this.req.reqseq, this.req.type, this.req.stype, []);
                this.channel.userphid.bridgeInput(this);
            } catch (e) {
                if (this.channel.userphid.onError) this.channel.userphid.onError(e);
        ...
        BridgePacket.prototype.get = function (name) {
            ...
            if (this.entries.hasOwnProperty(name)) return this.entries[name].v;
            return undefined;
        };
   ...
   * Device
   */
        var Device = function Device(conn, data) {
            ...
            if (this.type === 'VINT') this.vintID = data.vintID;
        };
   ...
   * Channel
   */
        var Channel = function Channel(conn, dev, data) {
            ...
            if (this.channelname.startsWith('Phidget')) this.cname = this.channelname.substring(7); else this.cname = this.channelname;
        ...
        Channel.prototype.getKey = function () {
            ...
            if (this.parent.type === 'VINT') key += this.parent.hubPort + '_' + this.isHubPort;
        ...
        Channel.prototype.match = function (phid) {
            ...
            if (phid.class !== this.class) return false;

            if (phid._serialNumber != undefined && phid._serialNumber !== -1) {
                if (phid._serialNumber != this.parent.serialNumber) return false;
            }

            if (phid._channel != undefined) {
                if (phid._channel != this.index) return false;
            }

            if (phid._hubPort != undefined && phid._hubPort !== -1) {
                if (phid._hubPort != this.parent.hubPort) return false;
            }

            if (phid._isHubPort !== undefined) {
                if (phid._isHubPort !== this.isHubPort) return false;
            }

            if (phid._deviceLabel !== undefined && phid._deviceLabel !== null) {
                if (phid._deviceLabel !== this.parent.label) return false;
            }
     ...
     */
        ...
        Channel.prototype.closing = function (phid) {
            ...
            if (!this.isopen) throw 'closing called when not attached; ch:' + this + ' phid:' + phid;
            if (this.userphid !== phid) throw 'closing called with wrong phid; ch:' + this + ' phid:' + phid;
   ...
   */
        Channel.prototype.close = function () {
            ...
            if (this.detaching) return Promise.resolve();
   ...
   */
        ...
        Phidget.prototype.open = function (_timeout) {
            ...
            if (this.useropen) return Promise.reject(new Error('Phidget already opened by user'));
            if (this.isopen) return Promise.reject(new Error('Phidget already opened'));;

            var self = this;

            if (_timeout) {
                var timeout = parseInt(_timeout);
                if (!isNaN(timeout)) this.openTimeout = timeout;
            }
        ...
        Phidget.prototype._close = function () {
            ...
            if (this.isopen) var closePromise = this.channel.close();
    ...
    */
            if (this.channel != undefined && !this.channel.detaching) {
                if (this.id in UserPhidgets) delete UserPhidgets[this.id];
            } else {
                this.useropen = true;
            }
            ...
            if (typeof closePromise !== 'undefined') return closePromise;
        ...
        Phidget.prototype.handleSetStatus = function (bp, version) {
            ...
            } if (this.data['_class_version_'] !== version) throw new Error(this.name + ' version (' + this.data['_class_version_'] + ') does not match (' + version + ')');
        };

        Phidget.prototype.checkOpen = function () {

            if (!this.isopen) throw "phidget is not open";
        };
        ...
        Phidget.prototype.setChannel = function (ch) {
            ...
            ch = parseInt(ch);
            if (!isNaN(ch)) this._channel = ch;
        };

        /* Phidget API */
        Phidget.prototype.getChannel = function () {

            if (this.channel == undefined) return this._channel;
        ...
        Phidget.prototype.getDeviceLabel = function () {
            ...
            if (!this.channel) return this._deviceLabel ? this._deviceLabel : '';
        ...
        Phidget.prototype.getDeviceSerialNumber = function () {
            ...
            if (!this.channel) return this._serialNumber;
        ...
        Phidget.prototype.getHubPort = function () {
            ...
            if (!this.channel) return this._hubPort;
        ...
        Phidget.prototype.getIsHubPortDevice = function () {
            ...
            if (!this.channel) return this._isHubPort;
        ...
        Phidget.prototype.getParent = function () {
            ...
            if (this.channel) return this.channel.parent;
            return undefined;
        };

        Phidget.prototype.isVINT = function () {

            if (this.channel) return this.channel.parent.type === 'VINT';
            return false;
        };
   ...
   * Manager
   */
        var Manager = function Manager(conn) {
        ...
        Manager.prototype.delete = function () {
            ...
            if (!this.conn) throw "Manager has been deleted";

            var idx = this.conn.Managers.indexOf(this);
            if (idx != -1) this.conn.Managers.splice(idx, 1);
            delete this.conn;
        };

        Manager.prototype.open = function () {

            if (!this.conn) return Promise.reject(new Error('Manager has been deleted'));
   ...
   * WebSocket onmessage
   */
        var Connection_ws_onmessage = function Connection_ws_onmessage(event) {
   ...
   */
        var Connection_sock_onmessage = function Connection_sock_onmessage(data) {
      ...
      */
        ...
        var Connection = function Connection(uri, cfg) {

            if (isNode) {
                var u = url.parse(uri);
                if (u.protocol === 'ws:') throw 'websocket not supported with node.js, use "phid://server:port"';
                if (u.protocol !== 'phid:') throw 'expected phidget protocol, use "phid://server:port"';
    ...
    */
            if (cfg) {
                ...
                if (cfg.onConnect && typeof cfg.onConnect === 'function') this.onConnect = cfg.onConnect;

                if (cfg.onClose && typeof cfg.onClose === 'function') this.onClose = cfg.onClose;

                if (cfg.passwd) this.passwd = cfg.passwd;
            }

            this.onattach = function (dev) {

                for (var m in this.Managers) {
                    if (this.Managers[m].isopen) this.Managers[m].onDeviceAttach(dev);
                }
            };
            ...
            this.ondetach = function (dev) {
                ...
                for (var m in this.Managers) {
                    if (this.Managers[m].isopen) this.Managers[m].onDeviceDetach(dev);
                }
            };
            ...
            this.onchannelattach = function (ch) {
                ...
                for (var m in this.Managers) {
                    if (this.Managers[m].isopen) this.Managers[m].onAttach(ch);
                } /*
       ...
       */
            ...
            this.onchanneldetach = function (ch) {
                ...
                for (var m in this.Managers) {
                    if (this.Managers[m].isopen) this.Managers[m].onDetach(ch);
                } ch.onDetach();
            };
    ...
    */
            this.onmessage = function (data, req) {
                ...
                if (!req) req = new Request(array);

                if (req.len > 0) {
                    if (isNode) var tmp1 = data.slice(req.hdrlen); else var tmp1 = new Uint8Array(array.buffer, req.hdrlen, req.len);
                    var tmp2 = String.fromCharCode.apply(null, tmp1);
                    data = JSON.parse(tmp2);
                }

                if (this.connected) return this.ondatamessage(data, req); else return this.onauthmessage(data, req);
            };
    ...
    */
        ...
        Connection.prototype.setKeepAlive = function (timeout) {
            ...
            if (isNaN(to)) throw new Exception('invalid keep alive:' + timeout);
        ...
        Connection.prototype.createManager = function (obj) {
            ...
            if (obj) {
                if (typeof obj === 'function') {
                    ...
                    if ('onDeviceAttach' in obj && typeof obj.onDeviceAttach === 'function') m.onDeviceAttach = obj.onDeviceAttach;
                    if ('onDeviceDetach' in obj && typeof obj.onDeviceDetach === 'function') m.onDeviceDetach = obj.onDeviceDetach;
                    if ('onAttach' in obj && typeof obj.onAttach === 'function') m.onAttach = obj.onAttach;
                    if ('onDetach' in obj && typeof obj.onDetach === 'function') m.onDetach = obj.onDetach;
                }
            }
        ...
        Connection.prototype.getChannel = function (chid) {
            ...
            if (this.Channels[chid] === undefined) throw new Error('invalid channel id:' + chid);
            return this.Channels[chid];
        };

        Connection.prototype.getDevice = function (phid) {

            if (!this.Devices[phid]) return null;
        ...
        Connection.prototype.send = function (hdr, data) {
            ...
            return new Promise(function (resolve, reject) {
                try {
                    if (self.websocket) {
                        if (!self.ws || self.ws.readyState != WebSocket.OPEN) reject(new Error('invalid websocket state'));
                        self.ws.send(hdr);
                        if (data.length > 0) self.ws.send(data);
                    } else {
                        if (!self.sock) reject(new Error('invalid socket'));
                        var buffer = new Buffer(hdr.byteLength);
                        for (var i = 0; i < hdr.length; i++) {
                            buffer[i] = hdr[i];
                        } self.sock.write(buffer);
                        if (data.length > 0) self.sock.write(data);
                    }
                } catch (ex) {
        ...
        Connection.prototype.getNextRequestSequence = function () {
            ...
            if (this.reqseq >= 65535) this.reqseq = 10;
            ...
            return new Promise(function (resolve, reject) {
                self.requests[reqseq] = {
                    ...
                    onTimeout: function onTimeout() {
                        reject(new Error('request ' + reqseq + ':' + repseq + ' ' + type + '/' + stype + ' timed out:' + tm() + ':' + self.requests[reqseq].time));
                    },
        ...
        Connection.prototype.maintainConnection = function () {
            ...
            if (this.connectionMaintainer !== undefined) return;

            this.connectionMaintainer = setInterval(function () {

                if (this.connected === true || this.stayopen !== true) return;
        ...
        Connection.prototype.connect = function () {
            ...
            if (this.connected === true) return Promise.resolve(this);
        ...
        Connection.prototype.onauthmessage = function (data, req) {
            ...
            if (this.authdata === undefined) console.log('packet recieved while not connected and authdata is not defined'); else this.authdata(data);
        };
    ...
    */
            if (req.flags & NR.Reply) {
                if (request === undefined) this.onError(root.PhidgetReturnCode.EPHIDGET_UNEXPECTED, 'No handler registered for reply:' + req);
                return;
            }
            ...
            switch (req.type) {
                ...
                case P22MSG.Channel:
                    ...
                    break;
                default:
                    var p = jPhidget_reject(root.PhidgetReturnCode.EPHIDGET_INVALID, 'Unknown request type:' + req.type);
            }
            if (p) {
        ...
        Connection.prototype.handleCommand = function (req, data) {
            ...
            switch (req.stype) {
                case P22SMSG.KeepAlive:
                    ...
                    break;
                default:
                    return jPhidget_reject(root.PhidgetReturnCode.EPHIDGET_UNEXPECTED, 'Unknown command subrequest:' + req.stype);
            }
        };
        ...
        Connection.prototype.handleDevice = function (req, data) {
            ...
            switch (req.stype) {
                ...
                default:
                    return jPhidget_reject(root.PhidgetReturnCode.EPHIDGET_UNEXPECTED, 'Unknown device subrequest:' + req.stype);
            }
        };
        ...
        Connection.prototype.handleDeviceAttach = function (req, data) {
            ...
            if (dev.phid in this.Devices) return jPhidget_reject(root.PhidgetReturnCode.EPHIDGET_DUPLICATE, 'duplicate device:' + dev);
        ...
        Connection.prototype.handleDeviceDetach = function (req, data) {
            ...
            if (dev !== -1) this.deviceDetach(dev);
        ...
        Connection.prototype.deviceDetach = function (dev) {
            ...
            if (!(dev.phid in this.Devices)) return jPhidget_reject(root.PhidgetReturnCode.EPHIDGET_NOENT, 'no such device:' + dev);
        ...
        Connection.prototype.handleChannel = function (req, data) {
            ...
            if (dev === null) return jPhidget_reject(root.PhidgetReturnCode.EPHIDGET_UNEXPECTED, 'missing channel parent');
        ...
        Connection.prototype.handshake = function () {
            ...
            return new Promise(function (resolve, reject) {
                ...
                _this2.send(req.buffer, json).catch(function (err) {
                    reject(err);
                });

                _this2.authdata = function (data) {
                    if (data.result !== 0) {
                    ...
                    this.authdata = function (data) {
                        ...
                        if (this.nonceC != data.nonceC) {
                            reject(new Error('Authentication Failure: nonce do not match (' + this.nonceC + ') vs (' + data.nonceC + ')'));
                            return;
                        }
   ...
   */
        ...
        function scanChannels(phid) {
            ...
            for (var c in Connections) {
                if (Connections[c].matchPhidget(phid)) return;
            }
        }
        ...
        function scanUserPhidgets(ch) {
            ...
            for (var ph in UserPhidgets) {
                ...
                if (phid.attaching === true || phid.isopen === true) continue;

                if (ch === undefined) {
                    if (scanChannels(phid) === true) return;
        ...
        setInterval(function () {
            for (var ph in UserPhidgets) {
                var phid = UserPhidgets[ph];
                if (phid.isopen) continue;
                if (phid.openTimeout == undefined) continue;
                if (tm() - phid.openTime > phid.openTimeout) phid.openTimedOut();
            }
        }, 500);
        ...
        AccelerometerBase.prototype._bridgeInput = function (bp) {
            ...
            switch (bp.vpkt) {
                ...
                case 0x2e:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('AccelerationChangeTrigger');
                    return true;
                case 0x36:
                    // BP_SETDATAINTERVAL
                    this.data.dataInterval = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('DataInterval');
                    return true;
            }
        };
        ...
        AccelerometerBase.prototype.getAcceleration = function () {
            ...
            if (this.isopen !== true) throw new Error("not attached");
        ...
        AccelerometerBase.prototype.setAccelerationChangeTrigger = function (accelerationChangeTrigger) {
            ...
            if (this.isopen !== true) return Promise.reject(new Error("not attached"));
        ...
        BLDCMotorBase.prototype._bridgeInput = function (bp) {
            ...
            switch (bp.vpkt) {
                ...
                case 0x28:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('Acceleration');
                    return true;
                case 0x36:
                    // BP_SETDATAINTERVAL
                    this.data.dataInterval = +bp.entries[0].v;
                ...
                case 0x8d:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('StallVelocity');
                    return true;
                case 0x2c:
                    // BP_SETBRAKINGDUTYCYCLE
                    this.data.targetBrakingStrength = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('TargetBrakingStrength');
                    return true;
                case 0x37:
                    // BP_SETDUTYCYCLE
                    this.data.targetVelocity = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('TargetVelocity');
                    return true;
            }
        };
        ...
        BLDCMotor.prototype.setStallVelocity = function (stallVelocity) {
            ...
            bp.send(this.channel, "BP_SETSTALLVELOCITY", function () {
                ...
                if (typeof this.setStallVelocitySuccess === "function") this.setStallVelocitySuccess(stallVelocity);
            }.bind(this));
        };
        ...
        CapacitiveTouchBase.prototype._bridgeInput = function (bp) {
            ...
            switch (bp.vpkt) {
                ...
                case 0x42:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('Sensitivity');
                    return true;
                case 0x2e:
                    // BP_SETCHANGETRIGGER
                    this.data.touchValueChangeTrigger = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('TouchValueChangeTrigger');
                    return true;
            }
        };
        ...
        CurrentInputBase.prototype._bridgeInput = function (bp) {
            ...
            switch (bp.vpkt) {
                ...
                case 0x2e:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('CurrentChangeTrigger');
                    return true;
                case 0x36:
                    // BP_SETDATAINTERVAL
                    this.data.dataInterval = +bp.entries[0].v;
                ...
                case 0x4a:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('PowerSupply');
                    return true;
            }
        };
        ...
        DCMotorBase.prototype._bridgeInput = function (bp) {
            ...
            switch (bp.vpkt) {
                ...
                case 0x28:
                    ...
                    return true;
                case 0x2a:
                    // BP_SETBACKEMFSENSINGSTATE
                    this.data.backEMFSensingState = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('BackEMFSensingState');
                    return true;
                case 0x33:
                    // BP_SETCURRENTLIMIT
                    this.data.currentLimit = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('CurrentLimit');
                    return true;
                case 0x77:
                    // BP_SETCURRENTREGULATORGAIN
                    this.data.currentRegulatorGain = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('CurrentRegulatorGain');
                    return true;
                case 0x36:
                    // BP_SETDATAINTERVAL
                    this.data.dataInterval = +bp.entries[0].v;
                ...
                case 0x3a:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('FanMode');
                    return true;
                case 0x2c:
                    // BP_SETBRAKINGDUTYCYCLE
                    this.data.targetBrakingStrength = +bp.entries[0].v;
        ...
        DictionaryBase.prototype._event = function (bp) {
            ...
            switch (bp.vpkt) {
                ...
                case 0x7c:
                    ...
                    break;
                case 0x80:
                    // BP_DICTIONARYREMOVED
                    this.handleRemoveEvent(bp);
                    break;
                case 0x7e:
                    // BP_DICTIONARYUPDATED
        ...
        if (typeof module !== 'undefined' && module.exports) module.exports.Dictionary = Dictionary;
        ...
        Dictionary.prototype.get = function (key, def /* optional */) {
            ...
            return new Promise(function (resolve, reject) {
                ...
                bp.send(self.channel, "BP_DICTIONARYGET").then(function (val) {
                    ...
                    if (def !== undefined) resolve(def, key); else reject(err);
                });
            });
        };
        ...
        DigitalInputBase.prototype._bridgeInput = function (bp) {
            ...
            switch (bp.vpkt) {
                ...
                case 0x40:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('InputMode');
                    return true;
                case 0x4a:
                    // BP_SETPOWERSUPPLY
                    this.data.powerSupply = +bp.entries[0].v;
        ...
        DigitalOutputBase.prototype._bridgeInput = function (bp) {
            ...
            switch (bp.vpkt) {
                ...
                case 0x37:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('DutyCycle');
                    return true;
                case 0x43:
                    // BP_SETLEDCURRENTLIMIT
                    this.data.LEDCurrentLimit = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('LEDCurrentLimit');
                    return true;
                case 0x44:
                    // BP_SETLEDFORWARDVOLTAGE
                    this.data.LEDForwardVoltage = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('LEDForwardVoltage');
                    return true;
                case 0x52:
                    // BP_SETSTATE
                    this.data.state = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('State');
                    return true;
            }
        };
        ...
        DistanceSensorBase.prototype._bridgeInput = function (bp) {
            ...
            switch (bp.vpkt) {
                ...
                case 0x2e:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('DistanceChangeTrigger');
                    return true;
                case 0x4b:
                    // BP_SETSONARQUIETMODE
                    this.data.sonarQuietMode = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('SonarQuietMode');
                    return true;
            }
        };
        ...
        EncoderBase.prototype._bridgeInput = function (bp) {
            ...
            switch (bp.vpkt) {
                ...
                case 0x38:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('Enabled');
                    return true;
                case 0x36:
                    // BP_SETDATAINTERVAL
                    this.data.dataInterval = +bp.entries[0].v;
                ...
                case 0x41:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('IOMode');
                    return true;
                case 0x2e:
                    // BP_SETCHANGETRIGGER
                    this.data.positionChangeTrigger = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('PositionChangeTrigger');
                    return true;
            }
        };
        ...
        Encoder.prototype.setPosition = function (position) {
            ...
            if (this.data.indexPosition != 1e300) this.data.indexPosition += position - this.data.position;
        ...
        FrequencyCounterBase.prototype._bridgeInput = function (bp) {
            ...
            switch (bp.vpkt) {
                ...
                case 0x3b:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('FilterType');
                    return true;
                case 0x40:
                    // BP_SETINPUTMODE
                    this.data.inputMode = +bp.entries[0].v;
            ...
            switch (bp.vpkt) {
                ...
                case PhidgetBase.prototype.BP_FREQUENCYDATA:
                    ...
                    if (counts == 0) {
                        ...
                        if (!this.data.hasOwnProperty('totalTicksSinceLastCount')) this.data.totalTicksSinceLastCount = 0;

                        if (Number.isNaN(this.data.totalTicksSinceLastCount)) return true;
        ...
        GPS.prototype.handleUnsupportedBridgePacket = function handleUnsupportedBridgePacket(bp) {
            ...
            switch (bp.vpkt) {
                ...
                case PhidgetBase.prototype.BP_TIME:
                    ...
                    if (typeof this.onTimeChange == 'function') this.onTimeChange(localHour, this.data["GPSTime.tm_min"], this.data["GPSTime.tm_sec"], this.data["GPSTime.tm_ms"]);
                    break;
                ...
                case PhidgetBase.prototype.BP_DATE:
                    ...
                    if (typeof this.onDateChange == 'function') this.onDateChange(this.data["GPSDate.tm_year"], this.data["GPSDate.tm_mon"], this.data["GPSDate.tm_mday"]);
                    break;
        ...
        HumiditySensorBase.prototype._bridgeInput = function (bp) {
            ...
            switch (bp.vpkt) {
                ...
                case 0x2e:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('HumidityChangeTrigger');
                    return true;
            }
        };
        ...
        IR.prototype.getLastCode = function () {
            ...
            if (typeof this.data.lastCodeKnown == "undefined") throw new Error("last code unknown"); else return { bitCount: this.data.lastCodeInfo.bitCount, code: this.data.lastCodeStr };
        };

        IR.prototype.getLastLearnedCode = function () {

            if (typeof this.data.lastLearnedCodeKnown == "undefined") throw new Error("last learned code unknown"); else return { code: this.data.lastLearnedCodeStr, codeInfo: this.data.lastLearnedCodeInfo };
        };
        ...
        LCDBase.prototype._bridgeInput = function (bp) {
            ...
            switch (bp.vpkt) {
                ...
                case 0x2b:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('Backlight');
                    return true;
                case 0x30:
                    // BP_SETCONTRAST
                    this.data.contrast = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('Contrast');
                    return true;
                case 0x34:
                    // BP_SETCURSORBLINK
                    this.data.cursorBlink = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('CursorBlink');
                    return true;
                case 0x35:
                    // BP_SETCURSORON
                    this.data.cursorOn = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('CursorOn');
                    return true;
                case 0x3e:
                    // BP_SETFRAMEBUFFER
                    this.data.frameBuffer = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('FrameBuffer');
                    return true;
                case 0x4e:
                    // BP_SETSCREENSIZE
                    this.data.screenSize = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('ScreenSize');
                    return true;
                case 0x50:
                    // BP_SETSLEEP
                    this.data.sleeping = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('Sleeping');
                    return true;
            }
        };
        ...
        LCDBase.prototype.copy = function (srcFramebuffer, dstFramebuffer, srcX1, srcY1, srcX2, srcY2, dstX, dstY, inverted) {
        ...
        LCD.prototype.setCharacterBitmap = function (font, character, bitmap) {
            ...
            if (fontSize.width <= 0) throw new Error("invalid arg (getFontSize): " + font);
            if (fontSize.height <= 0) throw new Error("invalid arg (getFontSize): " + font);
            ...
            return bp.send(this.channel, "BP_SETCHARACTERBITMAP", function () {
                if (typeof this.setCharacterBitmapSuccess === "function") this.setCharacterBitmapSuccess();
            }.bind(this));
        };
        ...
        LCD.prototype.getMaxCharacters = function (font) {
            ...
            switch (this.getDeviceID()) {
                case DeviceID.PHIDID_LCD1100:
                    if (parseInt(font) < 1 || parseInt(font) > 5) throw new Error("invalid arg (getMaxCharacters): " + font);
        ...
        LCD.prototype.getFontSize = function (font) {
            ...
            if (isNode == false) var global = window;
        ...
        LightSensorBase.prototype._bridgeInput = function (bp) {
            ...
            switch (bp.vpkt) {
                ...
                case 0x2e:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('IlluminanceChangeTrigger');
                    return true;
            }
        };
        ...
        MagnetometerBase.prototype._bridgeInput = function (bp) {
            ...
            switch (bp.vpkt) {
                ...
                case 0x2e:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('MagneticFieldChangeTrigger');
                    return true;
            }
        };
        ...
        MagnetometerBase.prototype.setCorrectionParameters = function (magField, offset0, offset1, offset2, gain0, gain1, gain2, T0, T1, T2, T3, T4, T5) {
        ...
        MotorPositionControllerBase.prototype._bridgeInput = function (bp) {
            ...
            switch (bp.vpkt) {
                ...
                case 0x78:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('DeadBand');
                    return true;
                case 0x39:
                    // BP_SETENGAGED
                    this.data.engaged = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('Engaged');
                    return true;
                case 0x3a:
                    // BP_SETFANMODE
                    this.data.fanMode = +bp.entries[0].v;
                ...
                case 0x88:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('Kd');
                    return true;
                case 0x8e:
                    // BP_SETKI
                    this.data.ki = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('Ki');
                    return true;
                case 0x87:
                    // BP_SETKP
                    this.data.kp = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('Kp');
                    return true;
                case 0x8d:
                    // BP_SETSTALLVELOCITY
                    this.data.stallVelocity = +bp.entries[0].v;
                    ...
                    return true;
                case 0x53:
                    // BP_SETTARGETPOSITION
                    this.data.targetPosition = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('TargetPosition');
                    return true;
                case 0x37:
                    // BP_SETDUTYCYCLE
                    this.data.velocityLimit = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('VelocityLimit');
                    return true;
            }
        };
        ...
        MotorPositionController.prototype.getAcceleration = function () {
            ...
            if (this.data.acceleration == 1e300) throw new Error("Acceleration not yet set"); else return this.data.acceleration * this.data.rescaleFactor;
        };
        ...
        MotorPositionController.prototype.setAcceleration = function (acceleration) {
            ...
            bp.send(this.channel, "BP_SETACCELERATION", function () {
                ...
                if (typeof this.setAccelerationSuccess === "function") this.setAccelerationSuccess(acceleration);
            }.bind(this));
        };
        ...
        MotorPositionController.prototype.setDeadBand = function (deadBand) {
            ...
            bp.send(this.channel, "BP_SETDEADBAND", function () {
                ...
                if (typeof this.setDeadBandSuccess === "function") this.setDeadBandSuccess(deadBand);
            }.bind(this));
        };
        ...
        MotorPositionController.prototype.setTargetPosition = function (targetPosition) {
            ...
            return bp.send(this.channel, "BP_SETTARGETPOSITION").then(function () {
                ...
                if (self.getEngaged() === true && self.getVelocityLimit() !== 0 && self.getPosition() !== self.getTargetPosition()) self.data.isMoving = true;
            });
        };

        MotorPositionController.prototype.getVelocityLimit = function () {

            if (this.data.velocityLimit == 1e300) throw new Error("Velocity limit not yet set"); else return this.data.velocityLimit * this.data.rescaleFactor;
        };
        ...
        PHSensorBase.prototype._bridgeInput = function (bp) {
            ...
            switch (bp.vpkt) {
                default:
                    return false;
                case 0x86:
                    // BP_SETCORRECTIONTEMPERATURE
                    this.data.correctionTemperature = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('CorrectionTemperature');
                    return true;
                case 0x36:
                    // BP_SETDATAINTERVAL
                    this.data.dataInterval = +bp.entries[0].v;
                ...
                case 0x2e:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('PHChangeTrigger');
                    return true;
            }
        };
        ...
        PowerGuardBase.prototype._bridgeInput = function (bp) {
            ...
            switch (bp.vpkt) {
                ...
                case 0x47:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('OverVoltage');
                    return true;
                case 0x38:
                    // BP_SETENABLED
                    this.data.powerEnabled = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('PowerEnabled');
                    return true;
            }
        };
        ...
        PressureSensorBase.prototype._bridgeInput = function (bp) {
            ...
            switch (bp.vpkt) {
                ...
                case 0x2e:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('PressureChangeTrigger');
                    return true;
            }
        };
        ...
        RCServoBase.prototype._bridgeInput = function (bp) {
            ...
            switch (bp.vpkt) {
                ...
                case 0x46:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('MinPulseWidth');
                    return true;
                case 0x45:
                    // BP_SETMAXPULSEWIDTH
                    this.data.maxPulseWidth = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('MaxPulseWidth');
                    return true;
                case 0x51:
                    // BP_SETSPEEDRAMPINGSTATE
                    this.data.speedRampingState = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('SpeedRampingState');
                    return true;
                case 0x53:
                    // BP_SETTARGETPOSITION
                    this.data.targetPosition = +bp.entries[0].v;
                ...
                case 0x37:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('Torque');
                    return true;
                case 0x55:
                    // BP_SETVELOCITYLIMIT
                    this.data.velocityLimit = +bp.entries[0].v;
                ...
                case 0x56:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('Voltage');
                    return true;
            }
        };
  ...
  */
        ...
        ResistanceInputBase.prototype._bridgeInput = function (bp) {
            ...
            switch (bp.vpkt) {
                ...
                case 0x2e:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('ResistanceChangeTrigger');
                    return true;
                case 0x4d:
                    // BP_SETRTDWIRESETUP
                    this.data.RTDWireSetup = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('RTDWireSetup');
                    return true;
            }
        };
        ...
        RFIDBase.prototype._bridgeInput = function (bp) {
            ...
            switch (bp.vpkt) {
                ...
                case 0x29:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('AntennaEnabled');
                    return true;
            }
        };
        ...
        SoundSensorBase.prototype._bridgeInput = function (bp) {
            ...
            switch (bp.vpkt) {
                ...
                case 0x2e:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('SPLChangeTrigger');
                    return true;
                case 0x74:
                    // BP_SETSPLRANGE
                    this.data.SPLRange = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('SPLRange');
                    return true;
            }
        };
        ...
        SpatialBase.prototype.setMagnetometerCorrectionParameters = function (magField, offset0, offset1, offset2, gain0, gain1, gain2, T0, T1, T2, T3, T4, T5) {
        ...
        StepperBase.prototype._bridgeInput = function (bp) {
            ...
            switch (bp.vpkt) {
                ...
                case 0x31:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('ControlMode');
                    return true;
                case 0x33:
                    // BP_SETCURRENTLIMIT
                    this.data.currentLimit = +bp.entries[0].v;
                ...
                case 0x39:
                    ...
                    return true;
                case 0x3f:
                    // BP_SETHOLDINGCURRENTLIMIT
                    this.data.holdingCurrentLimit = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('HoldingCurrentLimit');
                    return true;
                case 0x53:
                    // BP_SETTARGETPOSITION
                    this.data.targetPosition = +bp.entries[0].v;
        ...
        TemperatureSensorBase.prototype._bridgeInput = function (bp) {
            ...
            switch (bp.vpkt) {
                ...
                case 0x4c:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('RTDType');
                    return true;
                case 0x4d:
                    // BP_SETRTDWIRESETUP
                    this.data.RTDWireSetup = +bp.entries[0].v;
                ...
                case 0x2e:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('TemperatureChangeTrigger');
                    return true;
                case 0x54:
                    // BP_SETTHERMOCOUPLETYPE
                    this.data.thermocoupleType = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('ThermocoupleType');
                    return true;
            }
        };
        ...
        VoltageInputBase.prototype._bridgeInput = function (bp) {
            ...
            switch (bp.vpkt) {
                ...
                case 0x4f:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('SensorType');
                    return true;
                case 0x7a:
                    // BP_SETSENSORVALUECHANGETRIGGER
                    this.data.sensorValueChangeTrigger = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('SensorValueChangeTrigger');
                    return true;
                case 0x2e:
                    // BP_SETCHANGETRIGGER
                    this.data.voltageChangeTrigger = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('VoltageChangeTrigger');
                    return true;
                case 0x57:
                    // BP_SETVOLTAGERANGE
                    this.data.voltageRange = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('VoltageRange');
                    return true;
            }
        };
        ...
        VoltageOutputBase.prototype._bridgeInput = function (bp) {
            ...
            switch (bp.vpkt) {
                ...
                case 0x57:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('VoltageOutputRange');
                    return true;
            }
        };
        ...
        VoltageRatioInputBase.prototype._bridgeInput = function (bp) {
            ...
            switch (bp.vpkt) {
                ...
                case 0x38:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('BridgeEnabled');
                    return true;
                case 0x2d:
                    // BP_SETBRIDGEGAIN
                    this.data.bridgeGain = +bp.entries[0].v;
                    if (this.onPropertyChange) this.onPropertyChange('BridgeGain');
                    return true;
                case 0x36:
                    // BP_SETDATAINTERVAL
                    this.data.dataInterval = +bp.entries[0].v;
                ...
                case 0x2e:
                    ...
                    if (this.onPropertyChange) this.onPropertyChange('VoltageRatioChangeTrigger');
                    return true;
            }
        };
